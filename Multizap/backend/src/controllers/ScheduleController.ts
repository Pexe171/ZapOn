...
export const store = async (req: Request, res: Response): Promise<Response> => {
  const {
    ...
    sendAt,
    contactId,
    userId, // pode vir do front, mas vamos garantir o usuário logado
    ticketUserId,
    queueId,
    openTicket,
    statusTicket,
  ...
  const { companyId, id: loggedUserId } = req.user as any;

  // Garante que sempre teremos um userId para aplicar o timezone
  const effectiveUserId = userId || loggedUserId;
