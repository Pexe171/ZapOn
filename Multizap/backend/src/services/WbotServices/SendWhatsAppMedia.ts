...
const looksLikeAudioContainer = (ext: string, mimetype: string) => {
  ...
  // ✅ Ajuste 1: Removido para que .webm siga para o fluxo de vídeo
...
const SendWhatsAppMedia = async ({
  ...
  try {
    ...
    if (looksLikeAudioContainer(ext, mimeType)) {
      ...
      // ✅ Ajuste 2: Converter webm → mp4 no ramo de vídeo
      let finalPath = pathMedia;
      let finalMime = mimeType;

      // Se for webm, converte para mp4 (compatibilidade máxima)
      if (finalPath.toLowerCase().endsWith(".webm")) {
        try {
          const dir = ensureCompanyFolder(companyId);
          const mp4Path = path.join(dir, `${Date.now()}.mp4`);

          // ffmpeg: movflags faststart ajuda o vídeo a carregar rápido no WPP
          // pix_fmt yuv420p garante compatibilidade com players mobile
          const cmd =
            `"${ffmpegPath.path}" -y -i "${finalPath}" ` +
            `-movflags faststart -pix_fmt yuv420p "${mp4Path}"`;

          await new Promise<void>((resolve, reject) => {
            exec(cmd, (error) => (error ? reject(error) : resolve()));
          });

          // remove o webm original
          unlinkSync(finalPath);

          finalPath = mp4Path;
          finalMime = "video/mp4";
        } catch (e) {
          // Se falhar, tenta enviar o webm original mesmo
          Sentry.captureException(e);
        }
      }
