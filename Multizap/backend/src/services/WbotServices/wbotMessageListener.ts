...
const getBodyButton = (msg: any): string => {
  ...
  const interactive = msg?.message?.viewOnceMessage?.message?.interactiveMessage;
  const buttons = interactive?.nativeFlowMessage?.buttons;

  const isPix =
    Array.isArray(buttons) &&
    buttons.some(b => b?.name === "review_and_pay" || b?.name === "payment_info");

  if (isPix) {
    const btnWithParams = Array.isArray(buttons)
      ? buttons.find(b => !!b?.buttonParamsJson)
      : undefined;

    let pixPayload: any = null;

    if (btnWithParams?.buttonParamsJson) {
      try {
        pixPayload = JSON.parse(btnWithParams.buttonParamsJson);
      } catch (e) {
        pixPayload = null;
      }
    }

    bodyMessage = pixPayload
      ? `[PIX] ${JSON.stringify(pixPayload)}`
      : `[PIX]`;

    return bodyMessage;
  }

  // se quiser, trate botões normais aqui também
  const hasBodyText = interactive?.body?.text;
  if (hasBodyText) return `[BOTOES]`;
  ...
  if (isPix) {
    ...
    // Coloca o payload junto no body pra sua tela conseguir renderizar o "card"
    bodyMessage = pixPayload
      ? `[PIX] ${JSON.stringify(pixPayload)}`
      : `[PIX]`;
  ...
  // Se for interactive com botões mas não for PIX
...
// ✅ CORRIGIDO: agora é SINCRONO, corrige "=" e suporta "payment_info"
const getBodyPIX = (msg: proto.IWebMessageInfo) => {
  try {
    const interactive =
      msg?.message?.interactiveMessage ||
      msg?.message?.viewOnceMessageV2?.message?.interactiveMessage ||
      msg?.message?.viewOnceMessageV2Extension?.message?.interactiveMessage;
    ...
    // Procura botões relacionados a pagamento PIX (no seu log: payment_info)
    const paymentButton = buttons.find((b: any) =>
      ["payment_info", "review_and_pay", "pay", "pay_now"].includes(String(b?.name || "").toLowerCase())
    );

    if (!paymentButton?.buttonParamsJson) return null;

    let params: any = null;
    try {
      params = JSON.parse(paymentButton.buttonParamsJson);
    } catch (e) {
      // se vier inválido, devolve pelo menos algo visível no chat
      return "💳 Pagamento (PIX)\n\n(Erro ao ler dados do pagamento)";
    }

    const paymentSettings = Array.isArray(params?.payment_settings) ? params.payment_settings : [];
    const pixStatic = paymentSettings.find((p: any) => p?.type === "pix_static_code")?.pix_static_code;

    if (!pixStatic) {
      return "💳 Pagamento (PIX)\n\nNenhuma info de PIX encontrada.";
    }

    const merchant = pixStatic?.merchant_name || "—";
    const key = pixStatic?.key || "—";
    const keyType = pixStatic?.key_type || "—";

    // valor pode vir como { value, offset }
    let amountText = "";
    const total = params?.total_amount;
    if (total && typeof total === "object" && total.value !== undefined && total.offset !== undefined) {
      const v = Number(total.value || 0);
      const off = Number(total.offset || 100);
      const amount = off ? (v / off) : v;
      amountText = `\nValor: R$ ${amount.toFixed(2).replace(".", ",")}`;
    }

    const referenceId = params?.reference_id ? `\nRef: ${params.reference_id}` : "";

    return `💳 Pagamento (PIX)${amountText}${referenceId}\nBeneficiário: ${merchant}\nChave: ${key}\nTipo: ${keyType}`;
  } catch (err) {
    return "💳 Pagamento (PIX)";
  }
};
...
export const getBodyMessage = (msg: proto.IWebMessageInfo): string | null => {
  try {
    ...
    const types = {
      ...
      // ✅ CORREÇÃO: Formatar localização corretamente
      locationMessage: formatLocationMessage(msg.message?.locationMessage),
      liveLocationMessage: formatLiveLocationMessage(msg.message?.liveLocationMessage),
...
const formatLocationMessage = (locationMsg: any): string => {
  if (!locationMsg) return "📍 Localização";
  
  const latitude = locationMsg.degreesLatitude;
  const longitude = locationMsg.degreesLongitude;
  const name = locationMsg.name || "Localização";
  const address = locationMsg.address || "";
...
const formatLiveLocationMessage = (liveLocationMsg: any): string => {
  if (!liveLocationMsg) return "📍 Localização em tempo real";
  
  const latitude = liveLocationMsg.degreesLatitude;
  const longitude = liveLocationMsg.degreesLongitude;
  const caption = liveLocationMsg.caption || "";
  
  let formatted = `📍 Localização em tempo real\n`;
  formatted += `Latitude: ${latitude}\n`;
  formatted += `Longitude: ${longitude}`;
...
export const verifyMessage = async (
  ...
  // ✅ VERIFICAÇÃO ESPECIAL PARA LOCALIZAÇÃO
  let locationData = null;
  if (getTypeMessage(msg) === "locationMessage") {
    locationData = {
      latitude: msg.message?.locationMessage?.degreesLatitude,
      longitude: msg.message?.locationMessage?.degreesLongitude,
      name: msg.message?.locationMessage?.name || "Localização",
      address: msg.message?.locationMessage?.address || "",
      type: "location"
    };
  } else if (getTypeMessage(msg) === "liveLocationMessage") {
    locationData = {
  ...
  const messageData = {
    ...
    dataJson: JSON.stringify({
      ...msg,
      locationData // ✅ Inclui dados da localização
    }),
    ticketTrakingId: ticketTraking?.id,
    isPrivate,
    createdAt: new Date(
...
const isValidMsg = (msg: proto.IWebMessageInfo): boolean => {
  ...
  try {
    ...
    const ifType =
      ...
      // ✅ ADICIONADO: tipos de localização
