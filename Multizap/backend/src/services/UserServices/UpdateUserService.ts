...
interface UserData {
  ...
  timezone?: string; // <<< NOVO: fuso horário do usuário
}
...
const UpdateUserService = async ({
  ...
  // >>> NOVO: timezone – só atualiza se vier do front
  if (userData.timezone) { dataToUpdate.timezone = userData.timezone; }
  ...
  const serializedUser = {
    ...
    // >>> NOVO: devolve também o timezone para o select ficar na opção correta
