...
async function handleVerifySchedules(job) {
  try {
    const now = moment();
    const startWindow = now.clone().subtract(30, "seconds");
    const endWindow = now.clone();
    ...
    const { count, rows: schedules } = await Schedule.findAndCountAll({
      where: {
        ...
        sentAt: null,
        sendAt: {
          [Op.gte]: startWindow.toDate(), // 30s para trás
          [Op.lte]: endWindow.toDate()    // até agora
        }
      },
      include: [
    ...
    logger.info(
      `VerifySchedules -> janela ${startWindow.format(
        "YYYY-MM-DD HH:mm:ss"
      )} até ${endWindow.format("YYYY-MM-DD HH:mm:ss")} | encontrados: ${count}`
    );
...
async function handleSendScheduledMessage(job) {
  ...
  try {
    ...
    // ===== REAGENDAMENTO / RECORRÊNCIA (AJUSTADO) =====
    if (
      ...
      // Mantém a mesma hora e minuto do agendamento original
      const dataExistente = new Date(schedule.sendAt);
      const hora = dataExistente.getHours();
      const minuto = dataExistente.getMinutes();
      ...
      // Ajustes de dias úteis usando moment (sem gambiarra de timezoneOffset)
      function isDiaUtil(dateMoment) {
        const dayOfWeek = dateMoment.day();
        return dayOfWeek >= 1 && dayOfWeek <= 5; // 1 = segunda, 5 = sexta
      }

      function proximoDiaUtil(dateMoment) {
      ...
      function diaUtilAnterior(dateMoment) {
      ...
      let novaDataMoment = moment(novaData);

      if (schedule.tipoDias === 5 && !isDiaUtil(novaDataMoment)) {
        // somente dias úteis - volta para o dia útil anterior
        novaDataMoment = diaUtilAnterior(novaDataMoment);
      } else if (schedule.tipoDias === 6 && !isDiaUtil(novaDataMoment)) {
        // somente dias úteis - segue para o próximo dia útil
        novaDataMoment = proximoDiaUtil(novaDataMoment);
      }

      novaData = novaDataMoment.toDate();

      // Mantém a hora/minuto originais (sem mexer em timezoneOffset)
